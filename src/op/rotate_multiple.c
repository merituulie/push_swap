...
static void	rotate_both_stacks(t_rotate *bigger_r, t_list **bigger_l,
				t_rotate *smaller_r, t_list **smaller_l)
{
	int	extra_count;
	...
	{
		rotate_multiple_both(bigger_l, smaller_l,
			smaller_r->rotations, rev_rotate);
			...
			t_rotate *a_rotate, t_rotate *b_rotate)
{
	if (!a_rotate && !b_rotate)
		return ;
	if (!b_rotate || b_rotate->rotations == 0)
		rotate_one_multiple(a_stack, a_rotate);
	else if (!a_rotate || a_rotate->rotations == 0)
	...
	else
	{
		if (a_rotate->rev_rotate)
			rotate_multiple_both(a_stack, b_stack,
				a_rotate->rotations, rev_rotate);
		else
			rotate_multiple_both(a_stack, b_stack,
				a_rotate->rotations, rotate);
	}
}
